...
function LoginPage() {
  ...
  const { mutate: login, isPending, error } = useMutation({
    mutationFn: (data: LoginInput) =>
      apiClient
        .post<LoginResponse>('/admin/auth/login', data)
        .then((res) => res.data),
  ...
  return (
    <div className="w-full max-w-sm space-y-6">
      <div className="text-center">
        ...
        <p className="mt-1 text-sm text-gray-500">Platform administration — authorized staff only</p>
      </div>
